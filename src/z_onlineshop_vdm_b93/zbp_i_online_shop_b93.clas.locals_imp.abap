...
**ENDCLASS.
**
**CLASS lsc_ZI_ONLINE_SHOP_B93 DEFINITION INHERITING FROM cl_abap_behavior_saver.
**  PROTECTED SECTION.
**
...
**ENDCLASS.
**
**CLASS lsc_ZI_ONLINE_SHOP_B93 IMPLEMENTATION.
**
**  METHOD save_modified.
**    DATA: lt_online_shop_as TYPE STANDARD TABLE OF zshop_as_b93,
**          ls_online_shop_as TYPE zshop_as_b93.
**    IF zbp_i_online_shop_b93=>cv_pr_mapped-purchaserequisition IS NOT INITIAL.
**      LOOP AT zbp_i_online_shop_b93=>cv_pr_mapped-purchaserequisition ASSIGNING FIELD-SYMBOL(<fs_pr_mapped>).
...
**      INSERT zshop_as_b93 FROM TABLE @lt_online_shop_as.
**    ENDIF.
**
**  ENDMETHOD.
**
...
CLASS lsc_zbp_i_online_shop_b93 DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
    METHODS save_modified REDEFINITION.
ENDCLASS.

CLASS lsc_zbp_i_online_shop_b93 IMPLEMENTATION.
  METHOD save_modified.
    DATA : lt_online_shop_as TYPE STANDARD TABLE OF zshop_as_b93,
           ls_online_shop_as TYPE zshop_as_b93.
    IF zbp_i_online_shop_b93=>cv_pr_mapped-purchaserequisition IS NOT INITIAL.
...
CLASS lhc_zbp_i_online_shop_b93  DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
...
CLASS lhc_zbp_i_online_shop_b93  IMPLEMENTATION.
...
*  if a new package is ordered, trigger a new purchase requisition
      ...
      WITH VALUE #( ( %cid_ref = 'My%ItemCID_1'
                      %target  = VALUE #( (
                                          ...
                                          textobjecttype = 'B01'
                                          language       = 'E'
                                          plainlongtext  = 'item text created from PAAS API b93'
                                        ) (
                                          %cid = 'My%CCT_2'
                                          textobjecttype = 'B02'
                                          language       = 'E'
                                          plainlongtext  = 'item2 text created from PAAS API b93'
                                        ) )
              )   )
      ...
      zbp_i_online_shop_b93=>cv_pr_mapped = ls_pr_mapped.
    ENDLOOP.
...
* Update an existing info record
  ...
  METHOD calculate_order_id.
    DATA:
      online_shops TYPE TABLE FOR UPDATE zi_online_shop_b93,
      online_shop  TYPE STRUCTURE FOR UPDATE zi_online_shop_b93.

    SELECT MAX( order_id ) FROM zonlineshop_b93 INTO @DATA(max_order_id).

    READ ENTITIES OF zi_online_shop_b93 IN LOCAL MODE
       ENTITY Online_Shop
        ALL FIELDS
    ...
    MODIFY ENTITIES OF zi_online_shop_b93 IN LOCAL MODE
    ENTITY Online_Shop UPDATE SET FIELDS WITH online_shops
    MAPPED   DATA(ls_mapped_modify)
    FAILED   DATA(lt_failed_modify)
    REPORTED DATA(lt_reported_modify).

    DATA: lt_create_pr_imp TYPE TABLE FOR ACTION IMPORT  zi_online_shop_b93~create_pr,
